    ...
    def load(self) -> tuple[pd.DataFrame, np.ndarray]:
        ...
        if self.actors:
            ...
            for i, filename in enumerate(datasets_paths):  # for each csv file name
                if int(filename[filename.find(
                        "Actor_") + 6]) not in self.actors:  # if the actor number in the file name is not in self.actors
                    indices.append(i)  # append to indices the index to remove

            for index in sorted(indices,
                                reverse=True):  # sort descending the indices so that while removing the elements in the list we don't skip any.
        ...
        all_features = np.concatenate(all_features,
                                      axis=0)  # concatenate all the loaded features in a single numpy array.

        # load datasets
        dfs = []  # empty list that will contain the dataframes loaded from the selected csv fles.
        for _, filename in enumerate(
                tqdm(datasets_paths, desc="Loading csv datasets")):  # iterate over csv file names.
        ...
        if self.cams:
            ...
            cams_to_drop_mask = ~dataset["cam"].isin(
                self.cams)  # create a mask with false where the column "cam" of the dataset contains an unwanted cam number.
            ...
            all_features = np.delete(all_features, cams_to_drop_mask.tolist(),
                                     axis=0)  # remove unwanted features using the same unwanted cams mask.
            all_features = normalize(all_features, axis=1, norm="l1")  # normalize features using L1 norm.

        # drop off air frames
        if self.drop_offair:
            offair_mask = dataset[
                              "ar_labels"] == "actor_repositioning"  # create mask with False where ar label is "actor_repositioning".

            dataset = dataset.loc[~offair_mask,
                      :]  # drop "actor_repositioning" frames from dataframe using the created mask.
            dataset.reset_index(drop=True, inplace=True)  # reset dataframe index again.
            all_features = np.delete(all_features, offair_mask.tolist(),
                                     axis=0)  # delete actual features from all features vector using created mask.
...
def load_and_split(features_folder: str, dataset_folder: str, train_actors: list, val_actors: list, train_cams: list,
    ...
    if val_actors:  # if user inputs some validation actors use them as validation set.
        train_dataloader = DatasetLoader(dataset_folder, features_folder, train_actors, train_cams,
                                         drop_offair)  # istantiate DataLoader for train set.
        val_dataloader = DatasetLoader(dataset_folder, features_folder, val_actors, val_cams,
                                       drop_offair)  # istantiate DataLoader for val set.
        ...
        return normalize(X_train), y_train, normalize(
            X_val), y_val, cams_train, cams_val  # return normalized features, labels, and cam numbers.

    else:
        # do the train-validation split
        dataset_dataloader = DatasetLoader(dataset_folder, features_folder, train_actors, train_cams,
                                           drop_offair)  # istantiate DataLoader.
