...
class VideoSeqGenerator(Sequence):
    ...
    def get_video_seq(self, df) -> tuple[np.array, np.array]:
        ...
        for i in range(0, self.batch_size): # iterate over the batch_size
            ...
            assert len(labels_seq) > 0 # make sure that the mode operations have not deleted all the frames from the window.
                
            assert mode(l) == sequence_label #make sure that after removing the unwanted frames from the window, the mode of the labels is still the same.
            
            sequence_label = self.label_encoder.transform([sequence_label]).tolist()[0] # OneHotEncode the mode of the labels, a.k.a t window's label.
